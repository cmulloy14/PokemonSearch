...
struct PokemonListViewModelTests {
  ...
  @MainActor @Test func testInitialState() {
    ...
    #expect(viewModel.nextPageURL == URL(string:"https://pokeapi.co/api/v2/pokemon")!)
  ...
  @MainActor @Test func testFetchPokemonSuccess() async throws {
    ...
    mockService.mockResponse = PokemonListResponse(
      count: 1,
      next: URL(string:"https://pokeapi.co/api/v2/pokemon/?limit=20&offset=20")!,
      previous: nil,
    ...
    #expect(viewModel.nextPageURL == URL(string:"https://pokeapi.co/api/v2/pokemon/?limit=20&offset=20")!)
    #expect(!viewModel.isLoading)
  }
...
private class MockPokemonService: PokemonServiceProtocol {
  ...
  func fetchPokemonList(from url: URL?, completion: @escaping (Result<PokemonListResponse, any Error>) -> Void) {
